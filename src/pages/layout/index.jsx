...
export default function Layout() {
  ...
  useEffect(() => {
    ...
    if (savedSearch && window.location.pathname === "") {
      setSearchParams({ search: savedSearch }, { replace: true });
    }
  }, [setSearchParams]);
  ...
  useEffect(() => {
    ...
    // Komponent unmount bo‘lganda tiklash
    return () => {
      document.body.style.overflow = "";
    };
  }, [isSidebarOpen]);
  ...
  return (
    <div className="min-h-screen relative">
      {/* HEADER */}
      <header className="bg-[#249B73] pl-[400px] fixed w-full shadow-lg shadow-[#9aaba5] z-50 text-white h-14 flex justify-between items-center px-10 max-[945px]:px-8 max-[450px]:px-6 max-[400px]:px-4 max-[370px]:px-2">
        <div className="flex items-center !max-[950px]:hidden"></div>
        <div className="flex items-center gap-6 max-[600px]:gap-3 max-[500px]:gap-2 max-[945px]:justify-between max-[400px]:gap-1">
          <div className="flex items-center gap-6 max-[600px]:gap-3 max-[500px]:gap-2 max-[400px]:gap-1">
            <div className="relative">
              <form onSubmit={handleSubmit}>
                <input
                  onChange={handleSearchChange}
                  className="bg-white text-[14px] text-gray-600 outline-none pl-8 py-1.5 rounded-sm w-64 max-[945px]:w-50 max-[850px]:w-50 max-[400px]:w-40 max-[350px]:w-30"
                  type="text"
                  placeholder="Qidiruv..."
          ...
          <div className="relative" ref={menuRef}>
            <div
              className="flex items-center gap-2 cursor-pointer hover:bg-[#1e7d5d] px-2 py-1 rounded transition-colors max-[400px]:gap-[2px]"
            ...
            >
              ...
              <div className="flex items-center gap-1">
                <div className="text-left">
                  <p className="text-[12px] leading-4 max-w-[140px] truncate max-[580px]:hidden">
                    {profileData.last_name} {profileData.name}{" "}
                    {profileData.middle_name}
                  </p>
                  <p className="text-[11px] leading-4 text-white/80 max-[580px]:hidden">
      ...
      <div
        ...
        className={`transition-all duration-500 ease-in-out fixed z-60 bg-white py-4 top-3 left-3 h-screen rounded-2xl shadow-lg
    ${isSidebarOpen ? "w-[300px] xs:w-[200px]" : "w-[60px]"}
    overflow-y-auto`}
      >
        {/* X tugmasi faqat ochiq holatda ko‘rinadi */}
        {isSidebarOpen && (
          <button
        ...
        <div className="mt-4 flex flex-col gap-1 border-t border-gray-200 pt-4">
          {navList.map((item) => {
            // Agar admin bo'lmasa va item "Xodimlar" yoki "Statistika" bo'lsa, uni ko'rsatma
            if (
              !profileData.is_super_admin &&
              (item.name === "Xodimlar" || item.name === "Statistika")
            ) {
              return null;
            }
      ...
      <div className={`ml-[62px] h-[calc(100vh-56px)] p-5 pt-20`}>
        <Outlet
          context={{
            columnFilters,
            setColumnFilters: handleColumnFilterChange,
          }}
        />
      </div>
    </div>
